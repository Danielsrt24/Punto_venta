...
$filtro = $_GET['buscar'] ?? '';
$clientes = $cliente_model->readAll($filtro);
        ...
        <div class="row">
            <nav class="col-md-2 d-none d-md-block bg-light" style="min-height: 100vh;">
                <div class="position-sticky pt-3">
                    <ul class="nav flex-column">
                        <li class="nav-item">
                            <a class="nav-link" href="<?php echo $base_path; ?>/index.php?page=dashboard">
                        ...
                        <li class="nav-item">
                            <a class="nav-link active" href="<?php echo $base_path; ?>/index.php?page=clientes">
            ...
            <main class="col-md-10 ms-sm-auto px-4 py-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2><i class="bi bi-person"></i> Gestión de Clientes</h2>
                    <a href="<?php echo $base_path; ?>/index.php?page=clienteController&accion=crear" class="btn btn-primary">
                    ...
                    <div class="card-body">
                        <form method="GET" action="<?php echo $base_path; ?>/index.php" class="row g-2">
                            ...
                            <div class="col-md-10">
                                <div class="input-group">
                                    ...
                                    <input type="text" name="buscar" class="form-control" 
                                           placeholder="Buscar por nombre, teléfono, email, DUI o NIT..." 
                            ...
                            <div class="col-md-2 d-grid gap-2">
                                ...
                                <?php if(!empty($filtro)): ?>
                                <a href="<?php echo $base_path; ?>/index.php?page=clientes" class="btn btn-secondary btn-sm">
                                ...
                                <tbody>
                                    ...
                                    if ($clientes) {
                                        while($row = $clientes->fetch(PDO::FETCH_ASSOC)): 
                                            $hay_resultados = true;
                                            $tipo_cliente = !empty($row['TipoPerson']) ? $row['TipoPerson'] : 'NATURAL';
                                    ?>
                                    <tr class="<?php echo $row['estado'] == 0 ? 'table-secondary' : ''; ?>">
                                        <td><strong><?php echo htmlspecialchars($row['nombre']); ?></strong></td>
                                        <td><?php echo htmlspecialchars($row['DUI'] ?? 'N/A'); ?></td>
                                        <td><?php echo htmlspecialchars($row['NIT']); ?></td>
                                        <td><?php echo htmlspecialchars($row['telefono'] ?? 'N/A'); ?></td>
                                        <td><?php echo htmlspecialchars($row['email'] ?? 'N/A'); ?></td>
                                        <td>
                                            <span class="badge bg-<?php echo $tipo_cliente === 'JURIDICA' ? 'warning text-dark' : 'primary'; ?>">
                                                <?php echo htmlspecialchars($tipo_cliente); ?>
                                            </span>
                                        </td>
                                        <td>
                                            <div class="btn-group" role="group">
                                                <a href="<?php echo $base_path; ?>/index.php?page=clienteController&accion=editar&id=<?php echo $row['id']; ?>" class="btn btn-sm btn-warning" title="Editar">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                                <?php if($row['estado'] == 1): ?>
                                                    <a href="<?php echo $base_path; ?>/index.php?page=clienteController&accion=eliminar&id=<?php echo $row['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('¿Eliminar cliente?')" title="Eliminar">
                                                        <i class="bi bi-trash"></i>
                                                    </a>
                                                <?php else: ?>
                                                    <a href="<?php echo $base_path; ?>/index.php?page=clienteController&accion=editar&id=<?php echo $row['id']; ?>" class="btn btn-sm btn-success" title="Restaurar">
