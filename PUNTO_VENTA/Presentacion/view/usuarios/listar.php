...
$filtro = $_GET['buscar'] ?? '';
$usuarios = $usuario_model->readAll($filtro);
        ...
        <div class="row">
            <nav class="col-md-2 d-none d-md-block bg-light" style="min-height: 100vh;">
                <div class="position-sticky pt-3">
                    <ul class="nav flex-column">
                        ...
                        <li class="nav-item">
                            <a class="nav-link active" href="<?php echo $base_path; ?>/index.php?page=usuarios">
            ...
            <main class="col-md-10 ms-sm-auto px-4 py-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <?php if($rol === 'ADMINISTRADOR'): ?>
                    <a href="<?php echo $base_path; ?>/index.php?page=usuarioController&accion=crear" class="btn btn-primary">
                    ...
                    <div class="card-body">
                        <form method="GET" action="<?php echo $base_path; ?>/index.php" class="row g-2">
                            ...
                            <div class="col-md-2 d-grid gap-2">
                                ...
                                <?php if(!empty($filtro)): ?>
                                <a href="<?php echo $base_path; ?>/index.php?page=usuarios" class="btn btn-secondary btn-sm">
                                ...
                                <tbody>
                                    ...
                                    if ($usuarios) {
                                        while($row = $usuarios->fetch(PDO::FETCH_ASSOC)): 
                                            $hay_resultados = true;
                                            $es_admin = ($row['rol'] === 'ADMINISTRADOR');
                                    ?>
                                    <tr class="<?php echo $row['estado'] == 0 ? 'table-secondary' : ''; ?>">
                                        <td><strong><?php echo htmlspecialchars($row['usuario']); ?></strong></td>
                                        <td><?php echo htmlspecialchars($row['nombre']); ?></td>
                                        <td>
                                            <span class="badge bg-<?php echo $row['rol'] === 'ADMINISTRADOR' ? 'danger' : ($row['rol'] === 'SUPERVISOR' ? 'info' : 'primary'); ?>">
                                                <?php echo $row['rol']; ?>
                                            </span>
                                        </td>
                                        <td>
                                            ...
                                            <?php else: ?>
                                                <span class="badge bg-secondary">
                                                    <i class="bi bi-eye-slash"></i> Inactivo
                                                </span>
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                            <div class="btn-group" role="group">
                                                <a href="<?php echo $base_path; ?>/index.php?page=usuarioController&accion=editar&id=<?php echo $row['id']; ?>" class="btn btn-sm btn-warning" title="Editar">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                                <?php if(!$es_admin): ?>
                                                    <?php if($row['estado'] == 1): ?>
                                                        <a href="<?php echo $base_path; ?>/index.php?page=usuarioController&accion=eliminar&id=<?php echo $row['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('¿Eliminar usuario?')" title="Eliminar">
                                                            <i class="bi bi-trash"></i>
                                                        </a>
                                                    <?php else: ?>
                                                        <a href="<?php echo $base_path; ?>/index.php?page=usuarioController&accion=editar&id=<?php echo $row['id']; ?>" class="btn btn-sm btn-success" title="Restaurar">
                                                            <i class="bi bi-arrow-counterclockwise"></i>
                                                        </a>
                                                    <?php endif; ?>
                                                <?php else: ?>
                                                    <button class="btn btn-sm btn-secondary" disabled title="No se puede eliminar administradores">
